...
import { fold, slug, transliterateUk } from './index'
...
describe('transliterateUk — ДСТУ 9112:2021 System B (slug-grade)', () => {
	test('the standard headline cases', () => {
		...
		expect(transliterateUk('Україна, Хмельницький', 'dstu-9112')).toBe('Ukrajina, Khmeljnycjkyj')
		expect(transliterateUk('Згорани', 'dstu-9112')).toBe('Zghorany')
	})

	test('pangram (uklatn fixture)', () => {
		expect(transliterateUk('Щастям б’єш жук їх глицю в фон й ґедзь пріч.', 'dstu-9112')).toBe(
...
describe('slug', () => {
	...
	test('mixed-language and messy input', () => {
		expect(slug('  Björk & Гурт «Океан Ельзи»: LIVE!! ')).toBe('bjork-hurt-okean-elzy-live')
	...
	test('options: separator, lowercase, maxLength cuts at a word boundary', () => {
		expect(slug('Багато слів у назві', { separator: '_' })).toBe('bahato_sliv_u_nazvi')
		expect(slug('Kyiv Live', { lowercase: false })).toBe('Kyiv-Live')
		expect(slug('дуже довгий заголовок статті', { maxLength: 12 })).toBe('duzhe-dovhyi')
	})
})
